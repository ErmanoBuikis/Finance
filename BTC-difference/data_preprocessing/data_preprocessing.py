...
def preprocessing():
    ...
    file_csv_name = "/eurusd_input.csv" # <-- DA CAMBIARE con il nome giusto del csv di input
                                   # nome del file da analizzare.
                                   # I dati devono essere presenti 
                                   # nella stessa directory dello script python.

    new_file_name = sys.path[0] + "/eurusd_output.csv" # nome del nuovo file

    df = pd.read_csv(path_csv+file_csv_name) # prendi il CSV in input e lo trasformi in un dataframe 
    
    # pipeline 
    (df    
    .pipe(correct_error_type1)         # ERROR CORRECTION type 1
    .pipe(normalize_df)                  # NORMALIZE
    .pipe(date_time_formatting)
    )
    
    df.to_csv(new_file_name)              # Save into CSV
...
# usage
preprocessing() # generate output csv
